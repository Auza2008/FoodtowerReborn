...
public class SuperKnockback extends Module {
    private final Mode mode = new Mode("Mode", Modes.values(), Modes.Packet);
    ...
    private void onTick(EventAttack event) {
        if (event.getEntity() instanceof EntityLivingBase) {
            if (((EntityLivingBase) event.getEntity()).hurtTime > hurtTimeValue.get() || !timer.hasTimePassed(delay.get().longValue()) || (!MovementUtils.isMoving() && onlyMoveValue.get()) || (!mc.thePlayer.onGround && onlyGroundValue.get())) {
                return;
            }
